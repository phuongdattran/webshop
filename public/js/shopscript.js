    ...
    for(let i=0; i<cartAPI.length; i++) {
        ...
        tr.innerHTML = `
            <tr>
                ...
                <td class="quantity-box"><input type="number" id="quantity${cartAPI[i]._id}" size="4" value="${cart[i].number}" min="1" step="1" class="c-input-text qty text"></td>
                <td class="total-pr">
                    <p id="total${cartAPI[i]._id}">${((cartAPI[i].price/100)*cart[i].number).toFixed(2)}</p>
                </td>
                <td class="remove-pr">
                    <a href="#">
                        <i class="fas fa-times removeFromCart" id="remove${cartAPI[i]._id}"></i>
                    </a>
                </td>
            </tr>
        `;
...
if(window.location.href === "http://localhost:3000/cart/" || window.location.href === "http://localhost:3000/checkout/") {
    updatePrice();
}
